...
This is pretty much what you get when you generate a new Angular project nowadays. But without all the boilerplate and some basic styles.
...
Let's take a closer look in this `anonymise` method:
...
When the second effect is initially executed, a navigation will be triggered, but a navigation that has no data. Remember that, during the start up period of time, the `queryParamValue` signal is initialized to `undefined`. So this "empty" navigation will be triggered, cancelling Angular's start up navigation and taking over. This loses the actual values of the query params because the start up navigation is the one that contains all the data provided in the browser url when we load the application for the first time.
